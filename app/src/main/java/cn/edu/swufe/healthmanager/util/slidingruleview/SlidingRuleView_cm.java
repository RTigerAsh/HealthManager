package cn.edu.swufe.healthmanager.util.slidingruleview;
...
public class SlidingRuleView_cm extends View {
    ...
    public SlidingRuleView_cm(Context context) {
        this(context,null);

    }

    public SlidingRuleView_cm(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }


    public SlidingRuleView_cm(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //刻度线的颜色
        lineDegreeColor = typedArray.getColor(R.styleable.SlidingRuleView_Kg_lineDegreeColor, Color.LTGRAY);
        //顶部的直线距离View顶部距离
        topDegreeLine = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_topDegreeLine, SystemUtil.dp2px(getContext(),45));
        //刻度间隔
        lineDegreeSpace = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_lineDegreeSpace, SystemUtil.dp2px(getContext(),10));
        //刻度大数目 默认120
        lineCount = typedArray.getInt(R.styleable.SlidingRuleView_Kg_lineCount, 120);
        //长的刻度线条长度
        longDegreeLine = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_longDegreeLine, SystemUtil.dp2px(getContext(),35));
        //左间隔
        ruleLeftSpacing = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_ruleLeftSpacing, SystemUtil.dp2px(getContext(),5));
        //右间隔
        ruleRightSpacing = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_ruleRightSpacing, SystemUtil.dp2px(getContext(),5));
        //数字颜色
        numberColor = typedArray.getColor(R.styleable.SlidingRuleView_Kg_numberColor, Color.BLACK);
        //数字大小
        numberSize = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_numberSize, SystemUtil.dp2px(getContext(),15));
        //短刻度值的长度
        shortDegreeLine = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_shortDegreeLine, SystemUtil.dp2px(getContext(),20));
        //绿色指针粗细
        greenPointWidth = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_greenPointWidth, SystemUtil.dp2px(getContext(),4));
        //绿色指针颜色
        greenPointColor = typedArray.getColor(R.styleable.SlidingRuleView_Kg_greenPointColor, 0xFF4FBA75);
        //当前刻度的颜色
        currentNumberColor = typedArray.getColor(R.styleable.SlidingRuleView_Kg_currentNumberColor,
                0xFF4FBA75);
        //当前刻度的大小
        currentNumberSize = typedArray.getDimension(R.styleable.SlidingRuleView_Kg_currentNumberSize,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int i = 0;i <= lineCount * 10;i++){
            ...
            if(i % 10 == 0){
                ...
                //画刻度值
                String number = String.valueOf(i / 10+100);
                //得到文字宽度
        ...
        //确定数字的值。用移动多少来确定
        currentNum = df.format(((greenPointX + getScrollX() - leftBorder) / (lineDegreeSpace * 10.0f)+100));
        ...
        canvas.drawText("cm", greenPointX + textWidth / 2 + getScrollX() + SystemUtil.dp2px(getContext(),3), topDegreeLine - SystemUtil.dp2px(getContext(),30), paint);
    }
